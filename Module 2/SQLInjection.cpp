...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // convert to lowercase so that we can check later for lowercase "or"
    std::string lowercaseSql = sql;
    
    for (int i = 0; sql[i]; i++) {
        lowercaseSql[i] = tolower(sql[i]);
    }

    // checks if the string " or " exists in the query
    // checks if the query contains an equal sign
    // when these are used together, they are a sign of simple SQLi queries
    if ((lowercaseSql.find(" or ") != std::string::npos) && (lowercaseSql.find('=') != std::string::npos)) {
        std::cout << "\nSuspected SQL Injection detected:" << std::endl;
        std::cout << lowercaseSql << std::endl;
        return false;
    }
